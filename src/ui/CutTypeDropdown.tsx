...
export function CutTypeDropdown(props: CutTypeDropdownProps) {
  ...
  const icon = useMemo(() => {
    switch (cutType) {
      ...
      case 'online': return <IconOnLine16 />
  ...
  const options = cutType === 'Mixed'
    ? cutType
      ? shapeIsClosed ? mixedClosedCutTypeOptionsWithClear : mixedOpenCutTypeOptionsWithClear
      : shapeIsClosed ? mixedClosedCutTypeOptions : mixedOpenCutTypeOptions
    : cutType
      ? shapeIsClosed ? closedCutTypeOptionsWithClear : openCutTypeOptionsWithClear
      : shapeIsClosed ? closedCutTypeOptions : openCutTypeOptions
