...
import { coerceDimension, isDimension, parseRealDimensionString } from "../dimensions";
...
export function DimensionTextbox(props: DimensionTextboxProps) {
  const { initialValue, onValidInput, disabled, defaultUnits, ensurePositive, ...rest } = props
  ...
  const handleValidateOnBlur = useCallback((value: string) => {
    if (value.trim() === '') {
      return ''
    }
