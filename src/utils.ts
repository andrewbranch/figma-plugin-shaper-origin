...
import { CutType, FrameData, PathData, PathNode, PathSelection, SerializedNode, SerializedPath } from './types'

export function not<T>(predicate: (value: T) => boolean): (value: T) => boolean {
  return value => !predicate(value)
}

export function assertCutType(cutType: string, node?: BaseNode): asserts cutType is CutType {
  if (
    cutType !== 'inside' &&
    cutType !== 'outside' &&
    cutType !== 'online' &&
...
export function isGroupLikeNode(node: BaseNode): node is BaseNode & ChildrenMixin {
...
export function getPathSelection(selection: readonly BaseNode[]): PathSelection {
